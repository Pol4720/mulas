...
\node (partition) [draw, rectangle] {PARTITION};
\node (3partition) [draw, rectangle, below of=partition] {3-PARTITION};
\node (binpacking) [draw, rectangle, below of=3partition] {BIN PACKING};
\node (ourproblem) [draw, rectangle, below of=binpacking] {BALANCED-BIN-PACKING};

\draw[->] (partition) -- (3partition) node[midway, right] {$\leq_p$};
\draw[->] (3partition) -- (binpacking) node[midway, right] {$\leq_p$};
\draw[->] (binpacking) -- (ourproblem) node[midway, right] {$\leq_p$};
\end{tikzpicture}
\end{center}
...
\begin{corollary}[Inaproximabilidad]
