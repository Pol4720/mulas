...
\textit{(Cada flecha representa una reducción en tiempo polinomial)}
\end{center}
...
\[
DP[j][mask \cup S] = \min_{\substack{S \subseteq remaining \\ S \in Factible(j)}} \left\{ \max\left(DP[j-1][mask].V_{max}, V(S)\right) - \min\left(DP[j-1][mask].V_{min}, V(S)\right) \right\}
\]
